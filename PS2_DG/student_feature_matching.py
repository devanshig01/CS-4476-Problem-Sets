...
def match_features(features1, features2):
    ...
    #Compute all values of each row
    for row, arr in enumerate(dist):
        sample = []
        for index, value in enumerate(arr):
            sample.append((index, value))

        unsorted.append(sample)

    #Find the match and distance indices from each row sorted
    for index, arr in enumerate(unsorted):
        newArr = sorted(arr, key = lambda x:x[1])
        ratio = newArr[0][1] / newArr[1][1]
        confidence = 1 / ratio
        zipped.append((index, newArr[0][0], confidence))
    
    match = []
    confidence = []
    #create separate arrays and put them in
    for tups in zipped:
        x, y, c = tups
        match.append([x, y])
        confidence.append(c)
